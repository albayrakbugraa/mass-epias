...
CREATE TABLE MASS_WEBHOOK_KEYS (
    WEBHOOK_KEY_ID      NUMBER GENERATED BY DEFAULT AS IDENTITY (START WITH 1 INCREMENT BY 1) NOT NULL, -- Otomatik artan ID
    ...
    INVALIDATED_DATE    TIMESTAMP(6) WITH LOCAL TIME ZONE, -- Anahtar pasife alınma/zaman aşımı tarihi
    CONSTRAINT PK_MASS_WEBHOOK_KEYS PRIMARY KEY (WEBHOOK_KEY_ID)
);
...
COMMENT ON COLUMN MASS_WEBHOOK_KEYS.WEBHOOK_KEY_ID IS 'MASS Projesi Webhook anahtarı için benzersiz tanıtıcı.';
...
COMMENT ON COLUMN MASS_WEBHOOK_KEYS.CREATED_DATE IS 'Anahtarın oluşturulduğu tarih ve saat.';
COMMENT ON COLUMN MASS_WEBHOOK_KEYS.INVALIDATED_DATE IS 'Anahtarın pasife alındığı (veya süresi dolduğu) tarih ve saat.';
